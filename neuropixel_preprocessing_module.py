...
- Updated by TE in June 2024 to accomodate sorting and syncrhonization with Kilosort 4
"""
...
def align_data_streams(sync_files, data_stream_info, reference_stream, ks_ver):
    ...
    for ix, this_dir in enumerate(sync_dirs):
        ...
        if 'imec' in data_stream_info[ix]:
            ...
            ks_folder_name = 'ks' + ks_ver + '_out'
            spike_dir = Path(str(this_dir) + '/' + ks_folder_name + '/' + 'sorter_output')
...
def compute_basic_quality_metrics(results_dir, fs, rec_length):
    # Make spike table over entire session (each row is a different unit)
    spike_ids = np.load(results_dir / 'spike_clusters.npy').flatten()
    spike_times = 1000 * np.load(results_dir / 'spike_times.npy').flatten() / fs

    # Get spike times for each unit
    spike_table = []
    for uu in np.sort(np.unique(spike_ids)):
        spike_table.append(spike_times[spike_ids==uu]) # add unit-specific spike times to row

    spike_table = np.array(spike_table, dtype=object) # convert to numpy array

    # Compute firing rate, presence ratio, and ISI violations
    n_units = len(spike_table)

    isi_thresh = 1200 # ISI threshold (in us)

    firing_rate = np.zeros(n_units) # firing rates for each unit
    presence_ratio = np.zeros(n_units) # presence ratio (proportion of 1-minute time bins where there are spikes) for each unit
    pct_violations = np.zeros(n_units) # percentage of ISIs below threshold (i.e., ISI violations)

    for uu in range(len(spike_table)): # loop thru units and compute firing rate and 
        firing_rate[uu] = len(spike_table[uu]) / rec_length
        presence_ratio[uu] = np.mean(np.histogram(spike_table[uu], np.arange(0, 1e3 * rec_length, 60e3))[0] > 0)
        isis = 1e3 * np.diff(spike_table[uu]) # compute ISIs
        pct_violations[uu] = 100 * np.mean(isis < isi_thresh)
        
    # Make and save table of quality metrics computed above
    quality_metrics = pd.DataFrame({'firing_rate': firing_rate, 'presence_ratio': presence_ratio, 'pct_isi_violations': pct_violations})
    quality_metrics.to_csv(results_dir / 'quality_metrics.csv', index=False)